...
namespace MarkusSecundus.Utils.Physics
{
    ...
    public static class PhysicsHelpers
    {
        ...
        public static void SteerToVelocity(this Rigidbody self, Vector3 velocity, float maxDelta = float.MaxValue, ForceMode mode = ForceMode.VelocityChange)
        {
            var toApply = velocity - self.linearVelocity;
        ...
        public static void SteerToAngularVelocity(this Rigidbody self, Vector3 velocity, float maxDelta = float.MaxValue, ForceMode mode = ForceMode.VelocityChange)
        {
            var toApply = velocity - self.angularVelocity;
            toApply = Vector3.ClampMagnitude(toApply, maxDelta);
            self.AddTorque(toApply, mode);
        }
    }
